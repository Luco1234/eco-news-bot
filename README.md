...
### 2. Generate a Claude Code token (billed to your Pro subscription, not pay-per-token)
This uses your existing Claude Pro plan instead of separate API credits.
- On your own computer, install Claude Code if you haven't:
  `npm install -g @anthropic-ai/claude-code`
- Run: `claude setup-token`
- It opens a browser to log into your Claude.ai account (the one with your
  Pro subscription) and approve access.
- It prints a long token in your terminal — copy the whole thing. This is
  a **one-year token**, so you only need to do this once a year.
- This token can only be used to make Claude requests (nothing else), and
  its usage counts against your normal Pro plan usage, not a separate bill.
...
| Secret name | Value |
|---|---|
| `CLAUDE_CODE_OAUTH_TOKEN` | the token from `claude setup-token` in step 2 |
...
**Note on plan limits:** Pro plan usage limits are shared between your
claude.ai chats and this automation. One daily run is a very small, cheap
request, so it shouldn't meaningfully eat into your normal usage — but if
you ever hit a rate limit on claude.ai around the same time this runs, that's
why.
